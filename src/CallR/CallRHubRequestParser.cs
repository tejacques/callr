...
// This code copied from the SignalR github page in order
// to make minor tweaks to use the default JsonConvert
// configuration instead of custom ones.
...
namespace CallR
{
    internal class CallRHubRequestParser : IHubRequestParser
    {
        ...
        public HubRequest Parse(string data, JsonSerializer serializer)
        {
            ...
            request.ParameterValues = (deserializedData.Args != null) ? deserializedData.Args.Select(value => new CallRJRawValue(value)).ToArray() : _emptyArgs;
        ...
        private static IDictionary<string, object> GetState(HubInvocation deserializedData)
        {
            ...
            if (json.Length > 4096)
            {
                throw new InvalidOperationException("Error State Exceeded Maximum Length");
            }
